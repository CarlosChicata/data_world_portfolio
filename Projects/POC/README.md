...
| N° of Case | Title | Short description | Tools | Cloud Provider | Level | Concepts | Status |
|------------|-------|-------------------|-------|-------|---------------|----------|--------|
...
| 2 | GraphQL API of Serving layer from data lake | Create a GraphQL API to consume a data model is stored in data lake. | `AWS S3`, `AWS Athena`, `AWS AppSync`, `Python` | AWS |  1 / 5 |  `Data lake`, `Data modeling`, `AWS tools` | `Finished` 🤩 |
...
| 7 | Ingesting disaster tweets in real time | Processing tweets in real time and store in data model. | `Python`, `AWS Kinesis Data Firehose`, `AWS Redshift`, `Twitter data`, `AWS Textract` | AWS | 2 / 5 | `Real time processing`, ` Data ingestion`, `Analisis of Text` | `Get ready to start` |
